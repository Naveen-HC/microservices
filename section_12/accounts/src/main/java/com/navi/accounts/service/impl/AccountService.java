...
import com.navi.accounts.dto.AccountsMessageDto;
...
public class AccountService implements IAccountService {
    ...
    private void sendCommunication(Customer customer, Account account){
    AccountsMessageDto accountsMessageDto =  new AccountsMessageDto(account.getAccountNumber(),
            customer.getName(), customer.getEmail(), customer.getMobileNumber());
        LOGGER.info("Sending communication with details:{}", accountsMessageDto);
        boolean sent = streamBridge.send("sendCommunication-out-0", accountsMessageDto);
        LOGGER.info("Is communication is successfully proceeded? : {}", sent);
    }
    ...
    private Account createAccount(Customer customer){
        ...
        Long randomAccountNumber = 100000000000L + new Random().nextLong(90000000000L);
    ...
    public boolean updateCommunicationStatus(Long accountNumber) {
        ...
        account.setUpdateAccountsSw(true);
        Account savedAccounts = accountRepository.save(account);
        LOGGER.info("Account has updated: {}", savedAccounts);
        return  savedAccounts !=null;
    }
}
